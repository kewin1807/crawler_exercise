...
class DribbbleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        self.logger.info(scrapy_selector.css(
            '.vcard a[data-subject]::text').getall())
        self.logger.info(len(scrapy_selector.css(
            '.vcard a[data-subject]::text').getall()))

        # designer page with an infinite scroll
        # to get the current height of the page
        ...
        i = 0
        while i <= MAX_SCROLL:
            # to scroll to the bottom of the page and get the currently loaded page content
            ...
            self.logger.info("this is the current designer names extracted: {}".format(
                scrapy_selector.css('.vcard a[data-subject]::text').getall()))
            self.logger.info("Total names extracted: {}".format(
                len(scrapy_selector.css('.vcard a[data-subject]::text').getall())))
        ...
        self.logger.info("final designer names extracted: {}".format(
            scrapy_selector.css('.vcard a[data-subject]::text').getall()))
        self.logger.info("Final total names extracted: {}".format(
            len(scrapy_selector.css('.vcard a[data-subject]::text').getall())))

        # the following demostrates how to find the search location box
        # enter "New York" and then click the search button
        # search_location = driver.find_element_by_css_selector(
        #     '#location-selectized').send_keys('New York')
        # sleep(1)
