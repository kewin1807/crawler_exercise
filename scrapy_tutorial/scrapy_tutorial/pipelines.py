...
from scrapy_tutorial.models import create_table, db_connect, Quote, Author, Tag
from sqlalchemy.orm import sessionmaker
from scrapy.exceptions import DropItem
import logging

# create Pipeline to save items to the database
...
# create Pipeline to drop item duplicate
class DuplicatesPipeline(object):
    ...
    def process_item(self, item, spider):
        session = self.Session()
        exist_quote = session.query(Quote).filter_by(
            quote_content=item["quote_content"]).first()
